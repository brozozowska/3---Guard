...
calculateArea(x: 10, y: 50.5)
calculateArea(x: -10, y: 50.4)
...
firstNameTextField.text = "Sergey"
lastNameTextField.text = "Rozov"
...
func createUser() -> User? {
    ...
    guard lastNameTextField.text != nil else {
        return nil
    }
    ...
    let userInstance = User(firstName: firstNameTextField.text!, lastName: lastNameTextField.text!, age: ageTextField.text!)
    return userInstance
}
...
if let unwrappedFisrtName = createUser()?.firstName {
    print(unwrappedFisrtName)
}

if let unwrappedLastName = createUser()?.lastName {
    print(unwrappedLastName)
}

if let unwrappedAge = createUser()?.age {
    print(unwrappedAge)
}
/*:
